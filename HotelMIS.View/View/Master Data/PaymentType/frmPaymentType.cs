...
namespace HotelMIS.View
{
    public partial class frmPaymentType : Form
    {
        ...
        public frmPaymentType(PaymentType prmPaymentType = null)
        {
            InitializeComponent();
            oSession = SessionProvider.GetNewUnitOfWork();
            if (prmPaymentType != null)
                oPaymentType = oSession.GetObjectByKey<PaymentType>(prmPaymentType.Oid);
            ResetData();
        ...
        private void ResetData()
        {
            ...
            {
                oPaymentType = new PaymentType(oSession);
            }
            if (oSession.IsNewObject(oPaymentType))
            {
                oPaymentType.Code = String.Empty;
                oPaymentType.Name = String.Empty;
            }
            else
            {
                oPaymentType.Reload();
            }
            bs.DataSource = oPaymentType;
        }
        ...
        private void CheckCode()
        {
            ...
            if (oSession.FindObject<PaymentType>(PersistentCriteriaEvaluationBehavior.InTransaction,
              GroupOperator.And(new BinaryOperator("Oid", oPaymentType.Oid, BinaryOperatorType.NotEqual), new BinaryOperator("Code", txtCode.Text))) != null)
            {
