...
namespace HotelMIS.View
{
    public partial class frmStay : Form
    {
        ...
        public frmStay(Stay prmRoom = null)
        {
            InitializeComponent();
            SetEvent();
            oSession = SessionProvider.GetNewUnitOfWork();
            if (prmRoom != null)
                oStay = oSession.GetObjectByKey<Stay>(prmRoom.Oid);
            ResetData();
            LoadCombo();
            oErrorCollection = new List<ValidationClass>();
            if (oStay.IsReceiptPrinted)
            {
                DisableView();
            }
        }
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (isValidForm)
                        {
                            ...
                            if (oStay.TotalPayments != oStay.Total)
                            {
        ...
        private void btnCancelCheckIn_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            try
            {
                if (oStay.IsCheckOut == true || oStay.IsReceiptPrinted)
                {
                    return;
                }
                if (FormHelper.QuestionMessage("Are you sure want to cancel check in for this data ?"))
                {
                    if (oStay.Status != GlobalVar.TransactionStatus.Cancel)
                    {
                        ...
                        {
                            oStay.CancelRecord();
                            WorkingShiftDetail.CreateWorkingLog(oSession, "Cancel CheckIn " + oStay.ToString, 0);
                            oSession.CommitChanges();
                            this.Close();
                        }
                        else
            ...
            if (FormHelper.QuestionMessage("Are you sure want to print check in receipt for this data ?"))
                        {
                            ...
                            if (oStay.Status == GlobalVar.TransactionStatus.Entry)
                            {
                                ...
                                {
                                    PrintCheckInReceipt();
                                    oStay.IsReceiptPrinted = true;
                                    oStay.Save();
                                    WorkingShiftDetail.CreateWorkingLog(oSession, "Print Receipt " + oStay.ToString, 0);
                                    oSession.CommitChanges();
                                    this.Close();
                                }
                            }
        ...
        private void btnPayment_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            if (isValidForm)
            {
                ...
                oStay.UpdateRoomInformation(GlobalVar.RoomStatus.Filled);
                oSession.CommitChanges();
                frmPaymentVoucher oForm = new frmPaymentVoucher(oStay);
                oForm.ShowDialog();
            }
        }
